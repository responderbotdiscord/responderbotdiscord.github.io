...
window.wallpaperPropertyListener = {
    applyUserProperties: function (properties) {
        if (properties.speed) {
            global.speed = properties.speed.value;
        }
        if (properties.spawntime) {
            global.spawntime = properties.spawntime.value;
        }
        if (properties.backgroundlines) {
            global.backgroundlines = properties.backgroundlines.value;
            if (global.backgroundlines) document.body.getElementsByTagName("div")[0].style.visibility = "visible";
            else document.body.getElementsByTagName("div")[0].style.visibility = "hidden";
        }
        if (properties.discordlogo) {
            global.discordlogo = properties.discordlogo.value;
        }
        if (properties.backimagemode) {
            global.backimageview = properties.backimagemode.value;
            if (backimage) {
        ...
        if (properties.mouseinteraction) {
            global.mouseinteraction = properties.mouseinteraction.value;
        }
    }
}
function run() {
    if (frames < global.spawntime) frames++;
    else {
        circles.push({
    ...
    if (global.discordlogo) {
        //ctx.drawImage(discord_logo, 10, 10, 544/4, 195/4);
    }
    ...
    circles.forEach(circle => {
        circle.position.y -= circle.speed * (global.speed / 100);
    });
    ...
    if (preMouse.x != mouse.x && preMouse.x != mouse.x) {
        circles.forEach(circle => {
            circle.position.x += diffX / 25 * circle.speed * (global.speed / 100);
            circle.position.y += diffY / 25 * circle.speed * (global.speed / 100);
        });
    }

    if (diffLines > 5) diffLinesUp = false;
    else if (diffLines < -5) diffLinesUp = true;
    if (diffLinesUp) diffLines += 0.01 * (global.speed / 200);
    else diffLines -= 0.01 * (global.speed / 200);
    let addHeight = 0;

    addHeight = (-mouse.y / (canvas.height / 10) - 5) + diffLines;
